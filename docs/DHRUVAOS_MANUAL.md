...
| `gpio` | `gpio <pin> in\|out\|read\|write <0\|1>\|pull none\|up\|down` | General-purpose I/O (round 71) — direct BCM2835 GPFSEL/GPSET/GPCLR/GPLEV register access, pin 0-53. `pull` is real, spec-correct code but unverifiable under QEMU (real-hardware-only). |
| `fb` | `fb init <w> <h> <bpp>\|status\|fill <x> <y> <r> <g> <b>` | HDMI framebuffer (round 71) via the VideoCore mailbox property-channel interface — allocates a pixel buffer at the requested resolution/depth and writes pixels directly into it. No real HDMI signal to see under QEMU; the mode-set/pixel-write path itself is fully verified. |
| `hid` | `hid poll\|status` | USB HID boot-protocol keyboard/mouse (round 71) — one bounded interrupt-IN poll per `poll`, decoding whatever report comes back. Live-verified against real QEMU `usb-kbd`/`usb-mouse` devices, including real injected keypresses. |
